...
def train_one_epoch(fold, epoch, model, dataloader, optimizer, queue):
    ...
    for idx, data, labels in dataloader:
        ...
        print('Fold %d Epoch: %d Iter: %d' % (fold, epoch, iteration))
        iteration += 1

def validate_one_epoch(fold, epoch, model, dataloader, optimizer, logQ=None):
    model.eval()
    epoch_loss = []
    iteration = 0 
    for idx, data, labels in dataloader:
...
def train_fold(num_folds, num_epochs, fold, model_name, dataset_name, optimizer_name, learning_rate, num_classes, pretrain, label_column, logdir, lr_scheduler_name, queue):
    ...
    for epoch in range(num_epochs):
        ...
        # print("Im here3")
        # loss = validate_one_epoch(fold, epoch, model, validationloader, optimizer, logQ)
        # print("Im here4") 
