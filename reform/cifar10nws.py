...
def reform_datset(
    ...
    noisy_label_file = torch.load(osp.join(reform_dir, 'CIFAR-10_human.pt'))['worse_label']
    for idx, file_path in enumerate(data_files):
        file = open(file_path, mode='rb')
        content = pickle.load(file, encoding='bytes')
        file.close()

        batch_labels = noisy_label_file[idx * len(content[b'labels']):(idx + 1) * len(content[b'labels'])]
