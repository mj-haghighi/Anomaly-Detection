...
from utils.metrics import diff_two_max
from metric.level2.abstract_class import AbstractCumulativeMetricClass


class IntegralDiff2Max(AbstractCumulativeMetricClass):
    ...
    def calculate_metric_on_epochs(self, fold, phase):
        ...
        for epoch in range(self.epoch_skip, self.epochs):
            ...
            iterations_log[self.metric_name] = iterations_log['proba'].apply(lambda x: diff_two_max(x))
            iterations_log = iterations_log.drop(columns=['loss', 'proba'])
